...
public class WheelData
	...
	public PhysicMaterial lastPhysicMaterial = s_nonExistingMaterial;	// A non-existing physic material ensures the first iteration to match the changes.
	...
	// A non-existing physics material to initialize lastPhysicsMaterial with.
	//
	// Being static ensures an unique instantiation. Note that physics materials are NOT released by the GC,
	// and there's a limit in the physics materials that can be instantiated.

	public static PhysicMaterial s_nonExistingMaterial = new PhysicMaterial();
	}
...
public class VehicleController : MonoBehaviour
	...
	[NonSerialized] public bool useCustomGravity = false;
	[NonSerialized] public Vector3 customGravity = -9.81f * Vector3.up;
	...
	void OnEnable ()
		...
		float referenceDownforce = m_rigidbody.mass / m_wheelData.Length * 9.81f;

		foreach (Wheel wheel in wheels)
			{
			SetupWheelCollider(wheel.wheelCollider);
			UpdateWheelCollider(wheel.wheelCollider, referenceDownforce);
			wheel.wheelCollider.ConfigureVehicleSubsteps(1000.0f, 1, 1);
			}

		// Initialize other data

		m_lastImpactedMaterial = WheelData.s_nonExistingMaterial;	// A non-existing material to ensure cache missmatch at the first query
		}
	...
	void FixedUpdate ()
		...
		// Rigidbody: center of mass, gravity
		...
		float referenceDownforce = (m_rigidbody.mass * 9.81f) / m_wheelData.Length;
		...
		foreach (WheelData wd in m_wheelData)
			{
			if (!disallowRuntimeChanges)
				UpdateWheelCollider(wd.collider, referenceDownforce);
	...
	void ComputeGravity ()
		{
		m_gravity = useCustomGravity? customGravity : Physics.gravity;
		m_up = -m_gravity.normalized;
		m_gravityMagnitude = m_gravity.magnitude;
		}
	...
	void UpdateLocalFrame (WheelData wd)
		...
		if (surfaceForceRatio > 0.0f)
			...
			float upNormal = Vector3.Dot(m_up, wd.hit.normal);
			if (upNormal > 0.000001f)
				{
				Vector3 downForceUp = m_up * wd.hit.force / upNormal;
				surfaceForce = downForceUp - Vector3.Project(downForceUp, wd.hit.normal);
				}
			else
				{
				surfaceForce = m_up * 100000.0f;
				}
		...
		float estimatedSprungMass = Mathf.Clamp(wd.hit.force / 9.81f, 0.0f, m_rigidbody.mass / wheels.Length) * 0.5f;
	...
	void ApplyTireForces (WheelData wd)
		...
		if (wd.grounded)
			...
			if (otherRb != null && !otherRb.isKinematic)
				{
				float forceFactor = Mathf.Clamp01(otherRb.mass / m_rigidbody.mass);
				otherRb.AddForceAtPosition(-forwardForce * forceFactor, wd.hit.point);
				otherRb.AddForceAtPosition(-sidewaysForce * forceFactor, sidewaysForcePoint);
				}
			}
		}
	...
	public void ResetVehicle ()
		...
		m_rigidbody.MovePosition(m_rigidbody.position + m_up * 1.6f);
	...
	void EnableCollidersRaycast ()
		{
		// Enabled in reverse order so gameobjects with several colliders
		// get the correct layer (the first one saved for that gameobject).

		for (int i=m_colliders.Length-1; i>=0; i--)
	...
	void SetupWheelCollider (WheelCollider col)
		{
		WheelFrictionCurve colliderFriction = new WheelFrictionCurve()
			{
			stiffness = 0.0f
			};

		col.sidewaysFriction = colliderFriction;
		col.forwardFriction = colliderFriction;
		col.motorTorque = 0.00001f;
		}


	void UpdateWheelCollider (WheelCollider col, float minSuspensionForce)
		{
		JointSpring suspension = col.suspensionSpring;
		suspension.targetPosition = 0.0f;
		suspension.spring = Mathf.Max(suspension.spring, minSuspensionForce / col.suspensionDistance);
		col.suspensionSpring = suspension;

		if (col.sprungMass != 0.0f)
			col.sprungMass = 0.0f;
		}
